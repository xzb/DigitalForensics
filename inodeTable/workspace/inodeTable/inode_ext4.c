...
int main( int argc, char** argv )
{
	 ...
	 * ***************************************/
	struct ext4_super_block sb;			//not * here
    ...
    if ( ( read( fd, &sb, sizeof (struct ext4_super_block ) ) ) == -1 )
    {
        printf( "read error\n" );
        return -1;
    }
    ...
    if ( ( read( fd, &gd, sizeof (struct ext4_group_desc ) ) ) == -1 )
    {
        printf( "read error\n" );
        return -1;
    }
    ...
    printf("bg_inode_table = %u\n", gd.bg_inode_table_lo);		 //why +16 not +1?
...
//ext4 have _lo _hi , different from ext3, so it is very hard. ??
...
/*	struct ext4_inode inode;
    ...
    if ( ( lseek( fd, blockSize*(gd.bg_inode_table/16+1), SEEK_SET ) ) == -1 )	//not quite sure here?  block number begin from 0 or 1?   
	//block ID increased by 16, so? Problem of 64bit System?
    {
        printf( "lseek error\n" );
        return -1;
    }
    if ( ( read( fd, &inode, sizeof (struct ext4_inode ) ) ) == -1 )
    {
        printf( "read error\n" );
        return -1;
    }
